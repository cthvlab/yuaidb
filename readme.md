...
После каждой операции (`insert`, `update`, `delete`) , cначала записывается на диск в журнал в конец файла , затем каждые 60 секунд пишет в `.bin` файлы на диск — никаких "ой, забыл сохранить". 
